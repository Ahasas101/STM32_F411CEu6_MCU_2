/*
 * In this exercise we are entering the sleep mode using wait for EVENT
 * sleep mode
 */
...
int main(void)
{
  ...
  while(1)
  {
	  ...
	  // You must disable the systick interrupt else it will generate the interrupt every 1ms and will not let the processor to go to sleep
	  HAL_SuspendTick();

	  // going to sleep

	  // follow this order
	  __SEV(); // first you set the event flag
	  __WFE(); // clear the event flag
	__WFE(); // enable the WFE instruction
...
void button_init(void)
{
	...
	button.Mode = GPIO_MODE_EVT_RISING;
